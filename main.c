...
// Define structure to hold GPIO definitions
typedef struct {
    volatile int *mode;
    volatile int *idr;// ##########################################################
// ##########################################################
...
// Define array to hold GPIO definitions
GPIO_Definition gpio_definitions[] = {
    {&_SW0_MODE, &SW0},
    {&_SW1_MODE, &SW1},
    {&_SW2_MODE, &SW2},
    {&_SW3_MODE, &SW3},
...
int main() {
    // Loop over GPIO definitions and set them to 1
    for (int i = 0; i < sizeof(gpio_definitions) / sizeof(gpio_definitions[0]); i++) {
        *(gpio_definitions[i].mode) = 1;
        *(gpio_definitions[i].idr) = 1;
    }

    // Example of checking the values after setting them
    for (int i = 0; i < sizeof(gpio_definitions) / sizeof(gpio_definitions[0]); i++) {
        printf("Mode for pin %d: %u\n", i, *(gpio_definitions[i].mode));
        printf("IDR for pin %d: %u\n", i, *(gpio_definitions[i].idr));
    }
